...
class HomeController extends Controller
{
    ...
    public function SearchProduct(Request $request)
    {
        ...
        $products = Product::where('title', 'LIKE', "%$searchText%")
            ...
            ->orWhere('main_ingredients', 'LIKE', "%$searchText%")
            ->get();
    ...
    public function showPaymentForm($totalPrice)
    {
        return view('payment', ['totalPrice' => $totalPrice]);
    }

    public function processStripe(Request $request, $totalPrice)
    {
        // Validasi input
        $request->validate([
        ...
        // Logika untuk memproses pembayaran dengan Stripe
        \Stripe\Stripe::setApiKey(env('STRIPE_SECRET'));

        try {
            // Membuat token pembayaran
            $charge = \Stripe\Charge::create([
                'amount' => $totalPrice * 100, // dalam sen
                'currency' => 'usd',
                'source' => $request->stripeToken,
                'description' => 'Payment for order',
            ]);

            // Simpan informasi pembayaran ke database atau sesi
            Session::flash('success', 'Payment successful!');
        ...
        return redirect()->route('payment.form', ['totalPrice' => $totalPrice]);
    }
        ...
        //         'ewallet_type' => 'required_if:payment-method,ewallet',
        //         'bank_type' => 'required_if:payment-method,bank_transfer',
        //     ]);
        ...
        //     if ($request->input('payment-method') === 'ewallet') {
        //         $order->ewallet_type = $request->input('ewallet_type');
        //         // Tangani logika pembayaran e-wallet jika diperlukan
        //     } elseif ($request->input('payment-method') === 'bank_transfer') {
        //         $order->bank_type = $request->input('bank_type');
        //         // Tangani logika transfer bank jika diperlukan
        //     } elseif ($request->input('payment-method') === 'cod') {
        //         // Tangani logika pembayaran COD
        //     }
        ...
        //     return redirect()->route('checkout.success')->with('success', 'Success to checkout this product! Wait for this order to arrive at your home.');
        // }


        public function ewallet(Request $request, $totalPrice) {
            if (Auth::check()) {
                $user = Auth::user();
                $user_id = $user->id;
                ...
                // Simpan setiap item dalam keranjang sebagai pesanan
                foreach ($cartData as $data) {
                    $order = new Order();
                    $order->user_id = $data->user_id;
                    $order->name = $data->name;
                    ...
                    $order->payment_status = 'pending'; // Atau sesuai status pembayaran
                    $order->save();
                ...
                return redirect()->route('checkout.success')->with('success', 'Pembayaran berhasil dengan E-Wallet!');
            } else {
                return redirect('login');
            }
        }
        
        public function cod(Request $request, $totalPrice) {
            if (Auth::check()) {
                $user = Auth::user();
                $user_id = $user->id;
                ...
                foreach ($cartData as $data) {
                    ...
                    $order->tracking_id = 'TRK' . Str::limit(uniqid('', true), 15 - strlen('TRK'), '');
                    $order->delivery_status = 'processing';
                    $order->payment_status = 'cash_on_delivery'; // Status untuk COD
                    $order->save();
                ...
                return redirect()->route('checkout.success')->with('success', 'Your order has been received by COD method!');
            } else {
                return redirect('login');
            }
        }
        // public function ewallet($totalPrice) {
        //     // Proses pembayaran melalui E-Wallet
        //     // Simpan data ke database di sini
        
        //     return redirect()->route('checkout.success')->with('success', 'Pembayaran berhasil dengan E-Wallet!');
        // }
        
        // public function cod($totalPrice) {
        //     // Proses pembayaran COD
        //     // Simpan data ke database di sini
        
        //     return redirect()->route('checkout.success')->with('success', 'Pesanan anda telah diterima dengan metode COD!');
        // }
    }