...
class AdminController extends Controller
{
    ...
    {
        return view('admin.myaccount'); // Ganti dengan tampilan yang sesuai
    }
    ...
    public function AddProduct(Request $request)
    {
        if (Auth::check()) {
            ...
            if ($userType == 1) {
                ...
                $product->main_ingredients = $request->main_ingredients;
    ...
    {
        return view('admin.myaccount'); // Update path sesuai dengan lokasi file
    }

    // public function showStatistic() // Ubah nama fungsi di sini
    // {
    //     // Ambil data statistik penjualan dari tabel orders
    //     $sales_data = DB::table('orders')
    //         ->select(
    //             'product_title',  // Ganti product_name dengan product_title
    //             'quantity',       // Ambil quantity
    //             'price',          // Ambil price
    //             DB::raw('DATE(created_at) as sale_date')
    //         )
    //         ->get(); // Hapus groupBy jika tidak perlu
    
    //         return view('admin.statistic', compact('sales_data'));
    // }
    
    // public function downloadStatistic() // Ubah nama fungsi di sini
    // {
    //     // Ambil data statistik penjualan dari tabel orders
    //     $sales_data = DB::table('orders')
    //         ->select(
    ...
    //     // Buat file CSV
    //     $csvFileName = 'statistic_penjualan.csv';
    //     $handle = fopen('php://output', 'w');
    //     header('Content-Type: text/csv');
    //     header('Content-Disposition: attachment; filename="' . $csvFileName . '"');
    
    //     // Tulis header CSV
    //     fputcsv($handle, ['Nama Produk', 'Jumlah Terjual', 'Harga', 'Tanggal Penjualan']);
    
    //     // Tulis data ke CSV
    //     foreach ($sales_data as $sale) {
    //         fputcsv($handle, [
    //             $sale->product_title, // Ganti product_name dengan product_title
    //             $sale->quantity,      // Ambil quantity
    //             $sale->price,         // Ambil price
    //             $sale->sale_date,     // Tanggal penjualan
    //         ]);
    //     }
    ...
    public function showStatistic()
    {
        ...
        return view('admin.statistic', compact('data'));
    }
}
